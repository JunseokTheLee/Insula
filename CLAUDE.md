...
- **별자리 — 게임의 성취를 남기는 곳** (2026.9.21 사용자 지시, `supabase_stars.sql`·`js/constellations.js`·`js/stars.js`·`css/stars.css`·`{en,ko}/stars.html`). 두 게임 중 하나로 작품을 완성하면 **별 하나**가 켜진다(작품×게임 조합당 하나 — 같은 작품을 다시 해도 늘지 않고, 색칠은 더 높은 단계를 깨면 그 별이 커진다). 별 **6개**가 별자리 하나, 별자리 **6개**가 "밤" 하나다. 모양은 `CONSTELLATION_SHAPES` 12개를 순서대로 돌려 쓰고 밤마다 이름·색이 달라, **작품이 22점에서 100점(별 200개·별자리 33개)으로 늘어도 데이터 파일에 모양만 더하면 된다**(SQL 변경 없음 — 2026.9.21 사용자 확인).
  - **별 6개가 별자리 하나**(`STAR_CONSTELLATION_SIZE`, constellations.js) — 모든 모양이 정확히 6칸이고 결과 화면의 "다음 별자리까지 n개" 도 이 숫자로 센다. 바꾸면 이미 만들어진 별자리의 경계가 밀린다.
  - **고래는 없앴다** (2026.9.24 사용자 지시). 별 48개로 한 마리를 완성하던 그림(`WHALE_STARS`·`paintWhale`·`renderWhale`·`#stWhaleWrap`·`.st-whale*`·`starsWhale*` 문구·옵션 `starsShowWhale`)을 전부 지웠다. **별 데이터는 그대로다** — 고래는 별 목록으로 그리기만 했을 뿐 따로 저장하는 것이 없었다. DB 에는 쓰지 않는 `whale_stars()` RPC 와 옛 옵션 키만 남아 있고(지워도 되고 두어도 무해하다), 화면에는 별자리와 그 뒤 은하수만 남는다.
  ...
  - **화면 구성**(2026.9.21 사용자 지시로 한 번 다시 만들었다): ⓐ **진행 중인 별자리를 가운데 크게** — `.st-stage` 한 판(960×600 뷰박스, `preserveAspectRatio="xMidYMid slice"`)에 별자리 하나만 그리고, 패널 비율을 CSS 로 바꿔(모니터 16/10, 휴대폰 1/1·5/6) 좁은 화면에서는 빈 옆면을 잘라낸다 — 폭에 맞춰 축소하면 별이 6px 이 돼 아무것도 안 보였다 ⓑ 그 아래 **칩 줄**(`.st-strip`)에서 별자리를 골라 무대에 올린다
  ...
  - **전체 화면**(`#stFull`): "전체 별자리 크게 보기" 가 모든 별자리를 화면 모양에 맞춘 격자로 한 번에 그린다. 열 때 그리고 닫을 때 비우므로 닫혀 있는 동안 비용이 0이다.
  ...
  - **배경은 흐르고 별자리는 멈춰 있다** (2026.9.22 사용자 지시 "별자리는 그대로 있고, 배경 별들이 밝기가 변하면서 은하수처럼 살짝 흘러가게"). `scatter()`(stars.js)가 그리는 먼 별들은 ⓐ **밴드 5개**로 나뉘어 저마다 다른 주기로 밝아졌다 어두워지고 ⓑ 그 다섯을 담은 `<g class="st-way">` 가 **딱 한 화면 폭만큼** 왼쪽으로 흐른 뒤 처음으로 돌아간다 ⓒ 별을 `x` 와 `x + 폭` 두 곳에 그려 두어 이음매가 없다(바깥쪽은 SVG 뷰포트가 잘라낸다) ⓓ 별이 흘러갈 자리로 옅은 은하수 띠(`.st-haze`, 그라데이션 타원)를 깔았다. 속도는 폭과 무관하게 초당 7유저단위(`WAY_SPEED`)이므로 무대와 전체 화면이 같은 속도로 흐른다. **별자리의 별·선은 이 그룹 밖에 있어 움직이지 않는다.** 페이지 배경도 같은 원리로 두 층이 서로 반대로 아주 느리게 흐르며 번갈아 밝아진다(`st-field-a/b`·`st-field-dim`) — `background-position` 이 아니라 `transform` 이라 화면을 다시 칠하지 않고, 움직이는 층은 `inset:-64px` 로 작게 잡아 GPU 메모리를 아낀다.
  - **전체 화면이 열려 있는 동안 뒤쪽 애니메이션은 멈춘다** (`html.st-full-open`, 2026.9.22). 덮여서 보이지도 않는 무대와 페이지 배경이 계속 돌 이유가 없고, 마침 그때 화면에는 가장 무거운 하늘(별자리 전부)이 떠 있다. `#stFull` 안쪽은 id 선택자가 이겨서 그대로 돈다.
