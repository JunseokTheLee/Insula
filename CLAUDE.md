...
- **용어**: UI 의 "Save / Saving" 은 사용자 간 **팔로우**다 (`user_saves` 테이블, 네트워크 그래프의 바탕). `mosaic_collections` 는 **작가 포트폴리오**다 (2026.9.20 까지 "Collection/Exhibition(전시)" 이었다 — 테이블·컬럼 이름은 그대로). `cell_likes`·`cell_comments`·`profile_comments` 테이블은 예전 버전의 잔재로 현재 페이지 코드가 쓰지 않는다.
...
- `functions/{en,ko}/{campaigns,artworks,artists,portfolios}/[…].js` — 위 렌더러를 쓰는 상세 페이지 라우트 (`collections` 는 2026.9.20 `portfolios` 로 이름을 바꿨고 옛 주소는 `_redirects` 가 301)
...
- **정책 안에서 다른 테이블을 읽을 때는 그 테이블 정책이 되돌아오지 않는지 본다 — 되돌아오면 `security definer` 함수로 끊는다** (2026.9.20 실제 발생). `mosaic_submissions` select 정책이 `mosaic_collection_items` 를 서브쿼리로 읽고, `mosaic_collection_items` insert 정책이 다시 `mosaic_submissions` 를 읽자 항목 insert 마다 `42P17 infinite recursion detected in policy` 가 나 포트폴리오에 작품을 넣을 수 없었다. 정책 서브쿼리는 호출자의 RLS 아래에서 돌아 상대 테이블의 정책을 끌어들이므로, 서로 참조하는 두 테이블은 한쪽(또는 양쪽)을 `security definer`·`stable` 함수(`artwork_in_public_portfolio`, `artwork_is_own`)로 감싼다. select 만 있을 때는 드러나지 않고 **insert/update 때 터진다** — 새 정책은 익명 키로 insert 를 한 번 흉내 내 42501(정상 거부)이 오는지 확인한다.
...
- **2026.9.21 추가 SQL**: ㉒ `supabase_stars.sql`(**별자리** — `user_stars` 테이블(플레이어×작품×게임 한 행, **작품에 FK 를 걸지 않고** 제목·작가 이름을 사본으로 보관), `profiles.stars_hidden`, `game_best_records` insert 와 `pixel_progress` insert/update 에 붙는 별 지급 트리거 2개(`award_star`), 기존 완성 기록 백필, 읽기 RPC `user_star_list`·`artwork_star_counts`·`whale_stars`; **⑯·⑰·⑱·⑲·⑳ 뒤에 실행**. **2026.9.21 운영 DB 적용 확인 — 사용자 확인.** 고래를 없앤 2026.9.24 부터 `whale_stars()` 를 부르는 코드가 없다(함수는 DB 에 남아 있고, 두어도 무해하다).) 미적용이면 `/{lang}/stars` 가 안내문만 보이고 결과 화면의 별 표시·프로필 별자리 줄·작품의 "N명의 하늘에서 빛나는 중" 이 조용히 빠진다(게임 자체는 그대로). **작품에 FK 를 안 건 이유**: 두 게임의 완성 기록은 작품·캠페인이 지워지면 cascade 로 함께 사라지는데, 작가가 그림을 내렸다고 남의 밤하늘이 꺼지면 "남는다" 는 이 기능의 전제가 무너진다. 클라이언트에는 insert 권한이 없어 별을 스스로 줄 수 없다.
- **2026.9.21 추가 SQL**: ㉑ `supabase_pixel_progress_drop.sql`(숫자 색칠 "이어하기" 카드의 ✕ — `drop_pixel_progress(p_artwork_id, p_level)` RPC: 호출자 본인의 **미완성** 행만 삭제, 완성 행은 건드리지 않음(완성 수·작가 알림·프로필 스트립의 근거이므로); ⑳ 뒤에 실행. **2026.9.21 운영 DB 적용 확인 — 사용자 확인 + anon 실측: `drop_pixel_progress(1, 1)` 이 42501(정상 거부).**) 미적용이면 ✕ 가 이 기기의 진행만 지우고 서버 행은 남아 다음 접속 때 카드가 되돌아온다(토스트 "서버 기록은 지우지 못했어요"). 비로그인 플레이어는 기기 저장뿐이라 SQL 과 무관하게 동작한다.
- **2026.9.20 추가 SQL**: ⑳ `supabase_pixel_levels.sql`(숫자 색칠 **난이도 3단계**: `pixel_boards`·`pixel_progress` 에 `level` 컬럼과 복합 기본키 `(artwork_id, level)`·`(user_id, artwork_id, level)`, `set_pixel_board`·`save_pixel_progress` 를 level 인자 버전으로 교체(옛 시그니처 drop), `pixel_completion_counts` 는 플레이어 단위 집계, 프로필 스트립용 `pixel_user_levels`; **⑰·⑲ 뒤에 실행**. **2026.9.20 운영 DB 적용 확인 — 사용자 확인 + anon 실측: `pixel_boards.level`, `pixel_user_levels` 200, 새 `save_pixel_progress(p_artwork_id, p_level, p_filled)` 42501(정상 거부). 판 59개가 1·2·3단계(19/20/20)로 다시 만들어져 있음.**) 미적용이면 색칠 게임이 기기 저장 모드로만 돈다(판·진행이 서버에 저장되지 않음, 화면은 동작).
- **2026.9.20 추가 SQL**: ⑲ `supabase_portfolios.sql`(**Part A** 작품 공개/비공개, **Part B** 포트폴리오 — `mosaic_collections.cover_submission_id`·`preview_image_url`·`layout`, `mosaic_collection_items.position` 과 정렬 정책·grant, **항목 insert 정책을 "본인 작품만" 으로 교체**, `end_date` 비움; Part A: `mosaic_submissions.is_public`(기본 true)과 그 컬럼 grant, 조각이 원작의 공개 여부를 물려받는 트리거, 비공개 전환 시 칸을 돌려주고 공개 전환 시 재시도 표시를 지우는 트리거, **`mosaic_submissions` select 정책 교체**(공개 · 본인 · 관리자 · 공개/링크 공개 포트폴리오에 담긴 작품), `pixel_boards` select 정책(작품이 보일 때만), `match_pool_artworks` 재정의(비공개 조각 제외), `mosaic_stats` 뷰(공개 작품 수), `pixel_user_completions`·`game_medal_artworks`·`game_recent_runs` 재정의(비공개 작품 제외); **⑯·⑰·⑱ 뒤에 실행. ⑯·⑰·⑱ 중 하나라도 다시 실행하면 ⑲ 를 뒤이어 다시 실행한다.** **2026.9.20 운영 DB 적용 확인(정책 순환 수정판 재실행 포함) — 사용자 확인 + anon 실측: `is_public`·Part B 컬럼 존재, `artwork_in_public_portfolio` 호출 가능, `artwork_is_own`·`reorder_portfolio_items` 42501(정상 거부).**) 미적용이면 업로드의 "비공개" 선택이 "아직 준비되지 않았습니다" 로 거부되고(공개로 몰래 올리지 않는다), 라이트박스 편집 창의 비공개 행이 숨겨지며, 나머지 화면은 예전 그대로 동작한다.
- **2026.9.20 추가 SQL**: ⑱ `supabase_game_medal_rules.sql`(조각 찾기 메달 규칙: `profiles.ranking_excluded`·`game_best_records.voided_at/voided_by/void_note` 컬럼, `game_medal_min_players()`, **순위 RPC 7개 재정의**(`game_artwork_stats`·`game_medal_counts`·`game_top_players`·`game_medal_artworks`·`game_artwork_medals`·`game_my_standing`·`finish_game`), 최근 게임 목록 `game_recent_runs`, 관리자 `admin_set_ranking_excluded`·`admin_void_game_record`·`admin_game_records`; **⑯ 전체·⑪·③ 뒤에 실행. ⑯ `supabase_game.sql` 을 다시 실행하면 함수 본문이 옛 것으로 돌아가므로 그 뒤에 ⑱ 을 반드시 다시 실행한다.** **2026.9.20 운영 DB 적용 확인 — 사용자 확인 + anon 실측: `profiles.ranking_excluded`, `game_recent_runs(p_project_id, …)` 200.**) 미적용이면 화면은 그대로 동작하고(`medals` 플래그가 없으면 예전처럼 1~3위에 메달, 최근 게임 목록·관리자 "게임 기록"·"순위 제외" 는 안내문만) 규칙만 적용되지 않는다.
- **2026.9.20 추가 SQL**: ⑰ `supabase_pixel_game.sql`(숫자 색칠 게임: `mosaic_submissions.coloring_opt_out` 컬럼과 그 update grant, `pixel_boards`·`pixel_progress` 테이블, `set_pixel_board`·`save_pixel_progress`·`pixel_completion_counts`·`pixel_user_completions` RPC, **`notifications` 타입 제약 재정의**(`pixel_complete` 추가)와 완성 알림 중복 방지 인덱스; ⑨·③·⑫ 뒤, 그리고 **⑯·⑮ 뒤에 실행 — 세 파일이 같은 제약을 다시 만들며 이 파일의 목록이 상위 집합이다. ⑯이나 ⑮를 다시 실행했다면 ⑰을 뒤이어 다시 실행한다**(안 하면 완성 알림 insert 가 제약 위반으로 실패한다). **2026.9.20 운영 DB 적용 확인 — 사용자 확인 + anon 실측: `mosaic_submissions.coloring_opt_out`, `pixel_boards`.**) 미적용이면 `/{lang}/coloring` 은 **그대로 동작한다**: 판을 열 때 브라우저가 썸네일에서 즉시 변환하고 진행은 그 기기에만 남으며(목록 카드에 "열 때 변환"), 완성 수·작가 알림·프로필 "색칠한 작품"·관리자 "픽셀 판" 은 비활성이다. 적용 뒤 관리자 "작품 처리 → 픽셀 판 → 판 생성" 으로 기존 작품의 판을 한 번 만들고, Edge Function `push` 를 `--no-verify-jwt` 로 다시 배포해야 완성 푸시에 "색칠 완성 · 작품이름" 제목이 붙는다(재배포 전에는 제목 "Weavo"·빈 본문으로 나간다). **재배포 2026.9.21 완료 — 사용자 확인; 인증 없는 GET 에 게이트웨이의 401 이 아니라 함수의 405("method not allowed")가 오는 것으로 verify-jwt 꺼짐을 실측했다(부작용 없는 확인 방법 — GET 은 `claim_push` 에 닿지 않는다).**
...
- **숫자 색칠 게임(`/{lang}/coloring`, 2026.9.20)은 작품을 "판"으로 바꿔 칠한다** — `js/pixel-board.js` 의 `buildPixelBoard()` 가 480px 썸네일(`/img/`, 같은 출처라 캔버스 판독 가능)을 긴 변 N칸(옵션 `pixelGridSize`, 기본 48)으로 줄여 Lab 미디언 컷으로 K색(`pixelPaletteSize`, 기본 16)을 뽑고 ΔE<`pixelMergeDistance`(기본 8)인 색을 합친 뒤 칸마다 번호(밝은 색이 1번)를 매긴다. 색이 `pixelMinColors`(기본 6) 미만이거나 한 색이 90% 를 넘으면 판이 아니다(흰 스캔·단색). 판은 **작가가 업로드할 때**(`profile-view.js`, 조각 생성 직후)와 **관리자 "픽셀 판" 도구**가 `set_pixel_board` 로 저장해 모두 같은 판을 칠하게 하고, 저장된 판이 없으면 여는 사람의 브라우저가 즉시 변환한다(그 진행은 기기에만 저장 — 서버 저장은 판 행이 있어야 한다). **순위·시간 기록은 없다**: 서버는 칠한 정확성을 검증할 수 없으므로(완료 선언은 클라이언트) 완성 수·내 완성 목록·작가 알림만 둔다. 진행은 비트셋(칸당 1비트, base64)으로 `save_pixel_progress` 에 보내고 서버가 `bit_count` 로 세어 완성을 판정한다 — 칸 40개마다·4초 유휴·`pagehide`·`visibilitychange` 에 저장하고 `localStorage`(`weavo.coloring.progress.<id>`)에도 항상 남긴다. 판을 다시 만들면 `version` 이 올라 옛 진행은 버려진다. 화면은 캔버스 3장(색·강조·숫자)이며 DOM 칸을 만들지 않는다; 숫자 층은 화면상 칸이 15px 이상일 때만 보이고 "숫자 항상 보기" 토글로 고정할 수 있다. 작가 보호: 면책조항·개인정보처리방침에 변환 이용 문구가 있고, 작품 편집 창의 "숫자 색칠 게임에 쓰지 않음"(`coloring_opt_out`)으로 작품을 뺄 수 있다(기본 허용, 사용자 결정 2026.9.20). 완성물은 작품으로 등록되지 않는다. 진입 경로는 홈 히어로의 두 번째 배너(`data-coloring-link`), 게임 메뉴의 선택 화면, 라이트박스 "색칠하기" 버튼(`#lb-color-btn`), 프로필 "색칠한 작품" 스트립이다. **입력은 "스마트 드래그" 가 기본이다** (2026.9.20 사용자 결정, A안): 포인터가 **고른 번호의 빈 칸** 위에서 눌리면 스트로크가 시작돼 손가락을 따라 이어서 칠하고(포인터 샘플 사이는 선분 보간으로 빈칸 없이, 다른 번호 칸은 오답 표시 없이 조용히 지나감), 그 밖의 곳(다른 번호·이미 칠한 칸·여백)에서 눌리면 화면 이동이다. 두 손가락 핀치는 언제나 줌, 데스크톱은 가운데/오른쪽 버튼·Space+드래그가 이동. ✋ "이동 모드" 토글(`prefs.panMode`)을 켜면 예전처럼 탭으로만 칠한다. **스테이지의 pointerdown 은 줌 툴바 버튼 위에서는 아무것도 하지 않는다** — `setPointerCapture` 가 버튼의 click 을 삼켜 +/−/맞춤 버튼이 죽었던 원인(2026.9.20 수정). **난이도 3단계**(2026.9.20 사용자 지시, `supabase_pixel_levels.sql`): 작품마다 쉬움/보통/어려움 판이 따로 있고(`pixel_boards (artwork_id, level)`, 기본 24/40/64칸·10/13/16색 — 옵션 `pixelEasyGrid`…`pixelHardColors`), 진행도 판마다 따로다(`pixel_progress (user_id, artwork_id, level)`, 기기 저장 키 `weavo.coloring.progress.<id>.<level>`). 카드의 세 칩이 단계를 고르고, 큰 버튼은 "이어하던 단계 → 아직 안 한 가장 낮은 단계 → 어려움" 순으로 고른다. **목록 카드는 원본 썸네일을 보여주지 않는다** — 어려움 칸 수로 줄인 **회색 픽셀 판**(`previewInto`, 썸네일을 캔버스에 축소해 명도만 남김)이고, 본인이 완성한 작품만 **완성한 가장 높은 단계의 칸 수로 컬러** 픽셀 판이 된다. 판을 누르면 라이트박스(원본), 큰 버튼 옆 🖼 버튼도 원본 라이트박스. 목록 구조: 맨 위 "오늘의 작품"(원본 썸네일 그대로; 모두에게 같은 작품이되 내가 완성했으면 같은 순서의 다음 작품) → "이어하기"(진행 중인 작품, 최근순) → "완성한 작품" → "작품 둘러보기"(그 둘을 뺀 나머지를 방문마다 무작위 순서로, 24개씩 더 보기; 검색 중에는 섹션을 숨기고 검색 결과만 평면 목록). 판 생성(`makePixelBoardFor`)은 세 단계를 한 번에 만들고, 관리자 "픽셀 판" 은 세 판이 다 있는 작품만 완료로 센다. **PC(901px 이상)에서는 "오늘의 작품" 과 "이어하기 · 완성한 작품" 이 한 줄이다** (2026.9.20 사용자 지시): `.cg-top` 그리드의 왼쪽 300px 에 오늘의 작품(세로형 카드), 오른쪽에 두 블록이 가로 스트립(카드 184px, 넘치면 가로 스크롤 — `.hero-strip-track` 과 같은 `overscroll-behavior-x` 만)으로 붙고, 한쪽이 `hidden` 이면 `:has()` 로 남은 쪽이 전체 폭을 쓴다(오늘의 작품 혼자일 때는 예전 가로형). 900px 이하는 예전처럼 세로로 쌓인다. **2026.9.21 추가(사용자 지시)**: ⓐ 완성한 카드의 그림 왼쪽 위 📤 가 완성 그림을 공유한다 — 결과 화면의 "그림 공유" 와 같은 `sharePicture(a, b, btn)`(휴대폰은 공유 시트, PC 는 클립보드 복사 + 파일 저장)이며, 판은 메모리 → DB(`fetchPixelBoards`) → 썸네일에서 재생성(`boardFor`) 순으로 구한다 ⓑ 이어하기 카드의 왼쪽 위 ✕ 가 진행을 지운다(확인창 → 기기 저장 삭제 + 로그인 시 `drop_pixel_progress`; ㉑) — 완성 단계는 남고, 남는 것이 없으면 카드는 둘러보기 맨 앞으로 돌아온다 ⓒ 오늘의 작품에도 난이도 칩 3개(`#cgTodayLevels`, 같은 `levelChip`) ⓓ **플레이 화면의 원본 보기는 라이트박스가 아니라 "누르고 있는 동안만" 이다** — 툴바의 👁 버튼(또는 PC 측면 패널의 썸네일)을 누르고 있는 동안 원본 썸네일(`#cgPeek`, 캔버스와 같은 픽셀 크기·같은 transform)이 칸 위에 덮이고 떼면 사라진다(`wirePeek`: pointer capture, Space/Enter 도 같은 동작, contextmenu·drag 차단). 목록 카드의 🖼 은 그대로 라이트박스다. ⓔ **게임 안에서 뒤로 가기는 목록으로 돌아온다** (2026.9.21 사용자 지시, 1안): 판(게임)을 열 때 `screenHistory`(common.js)가 history 항목 하나를 push 하고, 뒤로 가기(브라우저·휴대폰 제스처·앱 뒤로 버튼 = `popstate`)가 오면 그 항목을 소비하며 색칠은 저장 후 목록으로, 조각 찾기는 플레이 중이면 ← 버튼과 같은 "그만둘까요?" 확인(취소하면 항목을 다시 push 해 그대로 머묾)·결과 화면이면 바로 목록으로 간다. 페이지 자체 버튼(←·목록으로)으로 나갈 때는 `endPlay` 가 `leave()` 로 그 항목을 스스로 pop 하므로 다음 뒤로 가기는 원래대로 이전 페이지다. `?artwork=`(·`?level=`)로 바로 연 경우 push 전에 그 쿼리를 지워 두어 뒤로 간 뒤 새로고침해도 목록이 뜬다(`?g=find` 는 남긴다). history 를 쓸 수 없는 환경이면 예전 동작 그대로다. ⓕ **쓸기는 다른 번호의 칸에 닿으면 끝난다** (2026.9.21 사용자 결정, 1안): 예전에는 다른 번호 칸을 조용히 건너뛰어, 번호를 찾지 않고 판 전체를 문지르기만 해도 고른 색이 다 칠해졌다. 이제 `paintAlong` 이 **아직 안 칠한 다른 번호 칸의 가운데 60%** 에 들어서면 `stopStroke` 가 그 쓸기를 끝낸다(짧은 틱 소리 `beep('tick')` + 그 칸의 붉은 깜빡임 `flashCell`, 첫 번째에 한 번 안내 토스트) — 손을 뗄 때까지 더 칠해지지도, 화면 이동으로 바뀌지도 않는다(`blocked`). 칠한 칸·빈 칸은 지나가도 되고 모서리를 스친 것은 봐준다(구간당 표본 3개). 방식은 사이트 옵션 `pixelDragWrongMode`(stop/mark/pass)로 고른다. 벌점·기록은 없다.
- **전시(컬렉션)는 작가 포트폴리오가 됐다** (2026.9.20 사용자 결정, `supabase_portfolios.sql` Part B). 페이지는 `{en,ko}/portfolio.html`(뷰어, `js/portfolio.js`·`css/portfolio.css`)과 `{en,ko}/portfolios.html`(디렉터리, `js/portfolios.js`), 라우트는 `/{lang}/portfolios/<id>`(Function `functions/{en,ko}/portfolios/[id].js`), 사이트맵은 `sitemap-portfolios.xml`. 옛 `/collections/*`·`/exhibitions` 는 `_redirects` 가 301. 규칙: ⓐ **자기 작품만** 담는다 — 피커는 `fetchOwnArtworkRows`, 라이트박스의 "포트폴리오에 담기" 는 작가 본인에게만 보이고, DB 의 insert 정책이 강제한다 ⓑ **공개 범위 3단계** — 공개(`is_public`+`is_published`, 목록·프로필·사이트맵에 실림) / 링크 공개(`is_public`만, 링크로만 열림, 서버·클라이언트 모두 `noindex`) / 비공개(본인만; RLS). 예전 "발행·종료일" 개념은 폐기(컬럼은 남김, `end_date` 는 SQL 이 비움). 빈 포트폴리오는 목록·사이트맵·타인 프로필에 나오지 않는다 ⓒ **순서·커버** — `mosaic_collection_items.position`(정렬 모드의 ↑↓ → `reorder_portfolio_items` RPC 한 번; 함수가 없으면 행마다 `position` 만 update), `cover_submission_id`(★). **`upsert` 로 저장하지 않는다** — PostgREST 는 충돌 시 payload 의 모든 컬럼(키 컬럼 포함)을 다시 쓰므로 `position` 만 허용한 컬럼 grant 에 걸려 "업데이트할 수 없습니다" 가 났다(2026.9.20 수정). 커버가 없으면 순서상 첫 작품 ⓓ **공유 카드** — 소유자의 브라우저가 제목·커버·작품 수를 바꿀 때마다 1.5초 뒤 1200×630 JPEG 를 캔버스로 그려 Storage 에 올리고 `preview_image_url` 에 기록한다(`renderShareCard`, 아바타는 교차 출처라 넣지 않음). Function 은 그 카드를 크기 태그와 함께 og:image 로, 없으면 커버 원본(크기 태그 없이), 그것도 없으면 사이트 카드를 쓴다(§5 규칙). "링크 복사" 는 터치 기기에서 `navigator.share`, 아니면 클립보드 ⓔ **뷰어 디자인(A안 다크 갤러리)** — `body.pf-dark` 가 공용 색 토큰을 어둡게 재정의해 상단 바·모달·라이트박스가 따라온다(흰 배경을 하드코딩한 입력창만 따로 덮음). 커버 평균색(color-engine)에서 `--pf-accent` 를 뽑고, 레이아웃은 `layout` 컬럼(grid/masonry/story), 슬라이드쇼(자동 6초·키보드·스와이프·전체화면), 스크롤 등장은 `prefers-reduced-motion` 을 따른다. 홈의 "최근 전시" 목록은 제거했고 헤더 메뉴는 "포트폴리오" 다. **인스타그램은 캡션 링크를 펼치지 않는다** — 공유 카드는 페이스북·X·카카오톡·스레드용이다.
- **휴대폰(≤640px)에서 포트폴리오 뷰어는 전체 화면이다** (2026.9.20 사용자 지시). 뷰어의 `<body>` 에만 있는 `pf-view` 클래스(디렉터리는 `pf-dark` 만)에서 상단 바·"← 포트폴리오" 링크·푸터를 숨기고, `.pf-backnav` 를 `position:fixed` 투명 막대로 바꿔 왼쪽에 **Weavo 홈 버튼**(`.pf-home`, PC 에서는 `display:none`), 오른쪽에 **소유자 도구**(`#pfOwnerTools` — `applyOwnerUI` 가 소유자가 아니면 `hidden`, 남에게는 홈 버튼 하나만 보인다)를 유리 원형 버튼으로 띄운다. 히어로는 그만큼 위 여백을 더 둔다(`.pf-hero` 72px + safe-area). 캠페인·네트워크의 `data-mobile-fs` 를 쓰지 않은 이유: 그 모드는 "←" 뒤로 버튼을 주입하는데 여기서는 홈 버튼 하나만 두기로 했고, 페이지는 보통대로 스크롤돼야 하기 때문이다. PC·태블릿(641px 이상)은 그대로다.
- **작품은 공개(기본) 또는 비공개다** (2026.9.20 사용자 결정, `supabase_portfolios.sql`). 비공개 작품은 **캠페인 모자이크에 들어가지 않는다** — 모자이크는 공개 화면이라 조각이 그대로 노출되기 때문이다. 업로드 때도 지금처럼 조각을 자르되 조각 행이 `is_public` 을 물려받아(BEFORE INSERT 트리거) 매칭 풀(`match_pool_artworks` 의 `sub.is_public`)에서 빠지고, 공개→비공개로 바꾸면 트리거가 놓인 조각을 칸에서 돌려주며(`unmatch_submission` 과 같은 문장), 비공개→공개로 바꾸면 `match_tried_at` 을 지워 다음 매칭 패스가 먼저 집는다. 클라이언트는 `is_public` 컬럼 update 한 줄만 한다(라이트박스 편집 창의 "비공개" 체크 — 공개 전환 뒤 `runPoolMatching()`). **읽기 권한은 RLS 가 정한다**: 공개 · 본인 · 관리자 · **공개/링크 공개 포트폴리오에 담긴 작품**(조각은 `parent_id` 로) — 그래서 비공개 작품을 공개 포트폴리오에 넣으면 그 포트폴리오를 보는 사람은 볼 수 있다(사용자 요구). 홈 최근 작품·탐색·색칠 목록·사이트맵은 `is_public = true` 를 명시해 **본인·관리자 화면에서도** 비공개 작품이 공개 목록에 섞이지 않게 하고(RLS 만으로는 본인 것이 보인다), 비공개 작품의 상세 URL 은 서버 Function 과 `artwork.js` 가 그 작품이 담긴 공개 포트폴리오로 302/replace 한다(없으면 404). 라이트박스는 비공개 작품에 게임·색칠 버튼을 숨기고 메타 줄에 "비공개" 를 붙인다. **한계(사용자 수용)**: 이미지 파일 URL 자체는 공개 버킷이라 알면 열린다 — 비공개 버킷·서명 URL 은 별도 과제. `pixel_boards`(48px 판)는 작품이 보일 때만 읽히는 정책으로 막았고, `set_pixel_board`·`save_pixel_progress` RPC 는 손대지 않았다(판이 이미 있고 id 를 아는 경우만 해당). 새로 작품 목록·집계를 만들 때 `parent_id is null` 과 함께 `is_public` 도 확인한다. 미적용(⑲) 상태에서 "비공개" 업로드는 파일을 올리기 전에 컬럼을 찔러 보고 거부한다 — 몰래 공개로 올리는 것이 유일하게 해서는 안 되는 일이기 때문이다. `queryWithOptional(build, optionals)`(common.js)이 이런 "있을 수도 없을 수도 있는 컬럼" 조회의 폴백을 한곳에서 처리한다.
- **조각 찾기 메달은 경쟁이 있어야 준다** (2026.9.20 사용자 결정, `supabase_game_medal_rules.sql`). 2026.9.20 실측에서 한 사람이 메달 32개 중 23개를 가졌는데 그중 17개는 그 작품을 혼자만 플레이해 얻은 1등이었다. 그래서 ⓐ 작품의 기록자가 `gameMedalMinPlayers`(기본 3)명 이상일 때만 1~3위가 메달이고 그 전에는 순번으로만 보인다(카드의 "기록자 3명부터 메달" 문구, 결과 화면·라이트박스·프로필·명예의 전당 모두 같은 RPC 규칙) ⓑ 관리자가 계정을 **순위 제외**(`profiles.ranking_excluded`)하면 기록은 남되 어떤 순위·메달·최근 게임 목록에도 나오지 않는다 ⓒ 관리자가 기록 한 건을 **무효 처리**(`voided_at`)하면 순위에서만 빠지고 되살릴 수 있으며, 같은 사람이 새 개인 최고를 내면 무효가 풀린다(무효는 그 한 판에 대한 판단 — 사람에 대한 조치는 ⓑ). 본인 작품 기록도 메달로 인정한다(사용자 결정). **데이터를 지우는 경로는 없다.** `finish_game` 의 "메달 변동" 알림도 같은 규칙(기록자 수 충족·제외 계정 아님)일 때만 나간다. 화면은 `medals` 플래그가 없으면(⑱ 미적용) 예전 동작으로 되돌아가고, 결과 화면은 `rank == null`(제외 계정)을 "3위 이내"로 읽지 않도록 `ranked` 가드를 둔다.
- **조각 찾기 목록에 "최근 게임" 이 있다** (2026.9.20, `game_recent_runs`). 명예의 전당 아래에 완주 기록 10건(옵션 `gameRecentRunsCount`, 더 보기 50건): 아바타·이름·작품 썸네일·시간(+힌트 n)·n분 전·메달(그 사람의 개인 최고이면서 현재 1~3위이고 규칙을 충족할 때만). 줄을 누르면 그 작품의 시작 확인창, 썸네일은 라이트박스. 결과 화면에는 그 작품의 최근 5건이 붙어 방금 기록이 어디쯤인지 보인다. 중간에 나간 판(`finished_at` 없음)·비로그인 플레이(세션 없음)·순위 제외 계정은 나오지 않는다. 옵션 `gameRecentRunsEnabled` 는 RPC 안에서도 검사한다. DB 사용량: 목록 뷰당 요청 +1(약 2.5KB), 결과 화면당 +1(약 1KB).
- **게임 메뉴 `/{lang}/game` 은 선택 화면으로 열린다** (2026.9.20 사용자 지시). 조각 찾기와 숫자 색칠 카드 두 장이 먼저 보이고, 조각 찾기의 데이터는 그 뒤에서 계속 읽어 `land()` 로 도착 화면(list/off/nocampaign/toofew)을 기억해 둔다. `?artwork=`(라이트박스·프로필 메달)나 `?g=find`(홈 배너·색칠 페이지의 전환 링크)로 오면 선택 화면을 건너뛴다. 링크 숨김 규칙: `data-game-link="find"` 는 조각 찾기가 꺼지면, `data-coloring-link` 는 색칠이 꺼지면, 값 없는 `data-game-link`(상단 메뉴 "게임")는 둘 다 꺼져야 숨긴다(`common.js hideGameLinksWhenOff`).
- **별자리 — 게임의 성취를 남기는 곳** (2026.9.21 사용자 지시, `supabase_stars.sql`·`js/constellations.js`·`js/sky-figures.js`·`js/stars.js`·`css/stars.css`·`{en,ko}/stars.html`). 두 게임 중 하나로 작품을 완성하면 **별 하나**가 켜진다(작품×게임 조합당 하나 — 같은 작품을 다시 해도 늘지 않고, 색칠은 더 높은 단계를 깨면 그 별이 커진다). **2026.9.26 부터 별자리는 이름 있는 13개이고 크기가 제각각이다** — 양자리(별 4개)·돌고래·게·염소·봉황·사자·백조·공작·토끼·황소·페가수스·물고기·전갈자리(17개), 모두 121개가 하늘 하나. **작은 것부터 차례로** 채워지고(사용자 결정), 13개를 다 채우면 같은 13개로 두 번째 하늘이 시작된다. 어느 별이 어느 별자리에 드는지는 **별의 순서만으로** 정해져 DB 에 저장하는 것이 없으므로, `CONSTELLATIONS` 의 순서·크기를 바꾸면 모든 사람의 하늘이 다시 그려진다(SQL 변경 없음).
  - **경계는 누적 크기로 센다** — `constellationSize()`·`groupIntoConstellations()`·`constellationProgress()`(constellations.js). 결과 화면의 "다음 별자리까지 n개"·요약 줄·프로필 줄이 모두 이 함수를 쓴다. 옛 `STAR_CONSTELLATION_SIZE`(6)·`CONSTELLATION_SHAPES`(12개 순환)·"밤" 은 없앴다.
  - **고래는 없앴다** (2026.9.24 사용자 지시). 별 48개로 한 마리를 완성하던 그림(`WHALE_STARS`·`paintWhale`·`renderWhale`·`#stWhaleWrap`·`.st-whale*`·`starsWhale*` 문구·옵션 `starsShowWhale`)을 전부 지웠다. **별 데이터는 그대로다** — 고래는 별 목록으로 그리기만 했을 뿐 따로 저장하는 것이 없었다. DB 에는 쓰지 않는 `whale_stars()` RPC 와 옛 옵션 키만 남아 있다(지워도 되고 두어도 무해하다).
  - **"서로 다른 우리" 배지는 없앴다** (2026.9.24 사용자 지시). 별 목록에서 여섯 가지 조건(서로 다른 작가 5·10명, 한 작품을 두 게임으로, 색칠 어려움, 하루에 3개, 별 20개)을 계산해 보여 주던 배지 줄을 코드·데이터·마크업·문구·스타일까지 모두 지웠다(`CONSTELLATION_SPECIAL`·`starBadges`·`badgeEmblem`·`renderSpecial`·`#stSpecialWrap`·`.st-badge*`·`conBadge_*`). **저장하던 데이터가 처음부터 없었으므로 DB 변경도, 잃는 기록도 없다** — 되살린다면 새로 만드는 셈이다.
  - **작가 쪽 성취**: 작품 라이트박스에 "N명의 하늘에서 빛나는 중"(`artwork_star_counts`). 이름은 밝히지 않고 숨긴 하늘도 센다 — 사람이 아니라 작품에 대한 숫자이기 때문이다.
  - 기본 공개이고 `profiles.stars_hidden` 으로 숨긴다. **순위·경쟁에는 쓰지 않는다** — 색칠의 정확성을 서버가 검증할 수 없다는 기존 원칙 그대로다.
  - 진입은 게임 선택 화면의 세 번째 카드, 프로필의 별자리 줄, 두 게임의 결과 화면(`#starAward`)이다. 상단 메뉴에는 넣지 않았다(이미 7개).
  - 브랜드 표기(로고 옆 ✦, "Pieces of People, A Brighter Sky")는 **별자리 페이지 안에서만** 쓴다(2026.9.21 사용자 결정) — 사이트 로고와 상단 바는 그대로다.
  - **화면은 끝없는 파노라마 한 장이다** (2026.9.26 사용자 지시). 밤하늘 사진 `sky/night-loop.webp`(1916×821)를 화면 높이에 맞추고(PC 78vh/78svh, 760px 이하 84svh; 넓은 화면에서는 끌 거리가 남도록 최대 1.10배까지 키워 사진 위쪽을 자른다) **옆으로 끌어 본다. 끝이 없다** — 오른쪽 끝을 지나면 왼쪽 하늘이 이어진다("360도 돌아보는 느낌"). 사진은 원본(`sky/ConstellationNightSkyBackground.png`, 커밋 안 함)의 양 끝 160px 을 겹쳐 섞어 이음매를 없앤 뒤 원래 폭으로 늘린 것이다(`node tools/sky-art.js <원본> sky/night-loop.webp --loop 160 --quality 82` — 약 9% 늘어나지만 별하늘에서는 보이지 않고, 덕분에 `sky-figures.js` 좌표를 하나도 바꾸지 않았다). 트랙(`#stSkyCanvas`)에 하늘을 두세 벌(`copies`, 화면이 사진보다 넓으면 더) 나란히 그리고 **스크롤이 아니라 transform 으로 옮긴다**(`panX` 는 늘 0 ≤ panX < 한 벌 폭). 마우스·펜·터치 모두 포인터 이벤트로 끌고 놓으면 관성으로 미끄러지며(동작 줄이기 설정이면 관성 없음), `touch-action:pan-y` 라 세로 스와이프는 그대로 페이지 스크롤이다. 가로 휠·트랙패드·Shift+휠은 하늘을 돌리고 **세로 휠은 가로채지 않는다**(위의 휠 회귀 참고). 포커스된 하늘은 ←/→ 로 4분의 1씩 움직인다. 캡션 카드의 ‹ › 도 13번 다음 1번으로 돈다. 13개는 사진 좌표에 놓이고(먼저 채우는 넷은 스크롤 없이 보이는 왼쪽, 백조·전갈은 실제처럼 은하수 위, y 640 아래 수평선에는 두지 않음), 아래 캡션 카드가 고른 별자리의 이름·문구·별 칸을 보인다. 끌던 위치는 새로고침·뒤로 가기일 때만 되살리고(`sessionStorage` `weavo.stars.view`, 한 벌 폭에 대한 비율), 끌기 안내는 한 번만 보인다(`localStorage` `weavo.stars.dragHint`). 13개를 모두 채운 사람에게는 "n번째 하늘" 버튼이 생긴다. **"크게 보기"** 는 같은 파노라마의 감싸개(`#stSkyWrap`)를 `position:fixed` 로 바꿀 뿐 따로 그리지 않는다(Esc 로 닫힘).
  - **완성 전에는 동물도 이름도 드러나지 않는다** (2026.9.26 사용자 지시, 두 번에 걸쳐). 채우는 중인 별자리와 아직 시작하지 않은 별자리는 **똑같이** 그림 불투명도 0.03(예전 옅은 상태 0.3 의 10%)이라 동물이 거의 보이지 않고, **이름도 감춘다** — 하늘 위 이름표는 완성된 별자리에만 붙고 채우는 중인 별자리에는 "5/9" 같은 진행 숫자만, 아직인 별자리에는 아무것도 없다. 캡션 카드도 이름·분류·문구 대신 "아직 드러나지 않은 별자리"(`starsConHidden`)와 "별 n개를 더 모으면 모습이 드러납니다"(`starsConHiddenLine`, 아직인 별자리는 `starsConLocked`)를 보이고, 화면 낭독기용 이름(`starsPickCon`)도 같다(`shownName()`, stars.js). 그래서 비로그인 방문자에게는 동물 없이 별과 옅은 선만 보인다. 페이지 소개 문구의 "양자리부터 전갈자리까지" 는 그대로다. 완성되면 밝아진다(0.94, 고르면 1). **완성되는 순간 한 번에 밝아진다** — 그 사람이 마지막으로 본 뒤 새로 완성된 별자리는 처음 볼 때 한 번, 그 별자리 색이 뒤에서 번지며(`.st-bloom`) 그림이 0.03 → 1 로 켜지고 선이 그어진다(`.reveal`). 몇 개가 완성돼 있었는지는 기기별·계정별로 `localStorage` `weavo.stars.seenDone.<id>` 에 두고, 처음 보는 기기에서는 완성된 것 전부가 한 번 켜진다. 그때 페이지는 방금 완성된 별자리를 가운데 두고 캡션을 그 별자리로 연다(한 하늘의 마지막 별자리였으면 그 하늘을 보인다). 남의 하늘에서는 연출하지 않는다.
  - **별을 누르면 작품 카드가 뜬다** (2026.9.26 사용자 지시). 누른 별 위에 작은 카드(썸네일·제목·작가·게임·"작품 보기 ›")가 고정되고, **카드를 누르면 작품이 열린다**(사이트의 다른 작품 격자처럼 라이트박스; 카드의 href 는 작품 페이지라 새 탭 열기도 된다). **별이 아닌 곳을 누르면** 카드가 사라지고, 하늘을 움직이거나 Esc 를 눌러도 사라진다. 마우스는 별에 올리기만 해도 카드가 미리 보인다(고정은 아님). 카드는 패널 밖으로 나가지 않게 좌우를 밀어 넣고, 위에 자리가 없으면 별 아래에 뜬다. 지워진 작품의 별은 카드를 누르면 "지금은 볼 수 없는 작품" 토스트.
  - **동물은 AI 그림이다** (2026.9.26 사용자 지시 — 반투명하게 빛나는 동물 참고 그림). 사용자가 만든 PNG(`sky/src/<key>.png`, **커밋하지 않음** — `.gitignore`)를 `node tools/sky-art.js sky/src` 가 투명 WebP(`sky/con-<key>.webp`, 긴 변 560px·품질 80, 13장 약 810KB)로 만든다. **이미 투명한 그림은 원본 알파를 그대로 쓴다** — 처음에 밝기로 알파를 다시 계산했더니 옅은 빛 번짐이 불투명해지고 파일이 두 배가 됐다. 검은 배경 그림만 밝기로 키잉하고, 알파는 4단계씩 반올림해 WebP 를 작게 한다. `sky-figures.js` 의 `img: { src, box }` 가 그림을 별자리의 0–100 상자에 놓고, **별 좌표(constellations.js)는 그림의 부위에 맞춰 찍었다**(사자는 가슴·갈기·이마·코, 황소는 얼굴의 V 와 두 뿔 끝 …). 그래서 **그림을 바꾸면 별도 다시 찍는다** — `tools/sky-editor.html`(크롬에서 파일로 바로 연다)에 그림을 끌어 놓고 옮기고 크기를 정한 뒤 별을 순서대로 찍으면 붙여 넣을 두 줄이 나온다. 바꾼 그림은 `src` 의 `?v=` 를 올린다(`/sky/*` 는 1주일 캐시, `_headers`). 그림을 못 불러오면 그 자리에는 별·선·이름만 남는다 — 선 그림 대체본은 두지 않았다(별이 그림에 맞춰져 있어 어긋난 대체본은 없느니만 못하다). 공유 카드(`shareCanvas`)도 같은 그림을 그린다(같은 출처라 캔버스를 내보낼 수 있다).
  - 반짝임은 전부 CSS 키프레임이다(라이브러리 없음). 별마다 애니메이션 지연을 다르게 줘 한꺼번에 깜빡이지 않게 하고, **`prefers-reduced-motion` 에서는 전부 멈춘다** — `css/stars.css` 맨 아래 목록에 새 애니메이션을 반드시 추가한다. **숨 쉬는 별은 두 요소다** — 위치는 감싸는 `<g transform="translate(…)">` 가, 크기 변화는 안쪽 path 의 CSS 애니메이션이 맡는다. CSS transform 은 같은 요소의 SVG `transform` 속성을 **덮어써서**, 한 요소에 둘 다 두었더니 반짝이는 별이 전부 하늘 왼쪽 위 구석으로 튀었다(2026.9.26). 별 위에 뜨는 작품 카드의 위치는 폭 비율이 아니라 `getScreenCTM()` 으로 잡는다(하늘이 확대·스크롤돼 있다).
  - **성능은 이 페이지의 기능 요구다** (2026.9.22 "버벅거린다"). ⓐ **SVG 필터를 하나도 쓰지 않는다** — `feGaussianBlur` 는 감싼 요소가 바뀔 때마다 다시 돌았다. 후광은 `radialGradient`, 별자리의 상태는 그림의 opacity 로만 나타낸다 ⓑ 트랙은 사진 `<img>` 몇 장과 사진 픽셀 좌표의 `<svg>` 한 장이고 transform 으로만 움직이므로 끌어도 다시 그리는 것이 없다 ⓒ 동물은 비트맵 13장이라 반투명 경로 수백 개보다 다시 칠하기가 싸다 ⓓ 숨 쉬는 별은 하늘 한 벌당 `ANIM_BUDGET`(24)개까지(무한 스크롤로 두세 벌을 그리므로 예전 페이지당 46개와 비슷한 양), 채우는 중인 별자리가 먼저 ⓔ 사진 위의 먼 별은 경로 3개(`.st-tw`)가 opacity 만 바꾼다(2026.9.22 의 흐르는 은하수 `st-way`·`st-haze` 는 사진이 은하수를 대신하면서 걷어냈다). 페이지 배경(`st-field-a/b`·`st-field-dim`)은 `background-position` 이 아니라 `transform` 으로 아주 느리게 흐른다. **여기에 필터를 다시 넣지 말 것.**
  - **크게 보기가 열려 있는 동안 뒤쪽 페이지 배경 애니메이션은 멈춘다** (`html.st-full-open`). 덮여서 보이지 않는 것이 계속 돌 이유가 없다.
...
- **배포 확인 때 새 `?v=` 주소를 HTML 보다 먼저 건드리지 않는다 (2026.9.20 실제 발생 — 확인 절차가 오염을 만들었다)**. push 직후 배포 확인용 curl 이 `portfolio.css?v=<새값>` 을 전환 전에 요청하자 엣지가 **옛 파일을 새 주소로 1년 캐시**했고, 그 뒤 휴대폰에 새 HTML + 옛 CSS 가 내려가 화면이 깨졌다(Weavo 홈 링크가 밑줄 글자로 노출, 전체 화면 미적용). 복구는 `node tools/bump-asset-version.js` 로 버전을 한 번 더 올려 커밋·push 하는 것뿐이다(캐시 퍼지는 Cloudflare 대시보드 권한이 필요). **순서**: ① HTML(no-cache)만 폴링해 새 `?v=` 가 보일 때까지 기다린다 ② 그 뒤 30초 이상 지나 새 주소의 CSS/JS 를 **한 번** 받아 확인한다. `?v=` 없는 맨 주소는 1년 캐시된 옛 파일이 오므로 확인에 쓰지 않는다.
...
- **화면은 왼쪽 탭 레일 + 패널 구조다** (2026.9.11). 탭은 대시보드(방문 통계 → 사용량) · 신고 · 작품 · 댓글 · 캠페인 · 작품 처리(풀 대기·조각·썸네일·픽셀 판) · 게임 기록(기록 조회·무효 처리·메달 현황) · 회원(관리자 목록·업로드 차단) · 공지 · 사이트 옵션 · 기록. **탭을 처음 열 때만 그 탭의 데이터를 읽는다**(예전에는 진입하자마자 9개 요청이 한꺼번에 나갔다). 현재 탭은 주소 해시(`/ko/admin#artworks`)에 남아 새로고침·북마크에도 유지된다. 새 관리 기능은 새 `<section>` 을 알맞은 패널 안에 넣고 `ADMIN_TAB_LOADERS` 에 로더를 건다.
...
- **게임 기록 탭 · 순위 제외 (2026.9.20, `supabase_game_medal_rules.sql`)** — "게임 기록" 탭은 조각 찾기 개인 최고 기록 전부를 최신순(50건씩, 이름·작품 제목 검색)으로 보여주고 체크한 기록을 **무효 처리/되살리기** 한다(20건 상한·무효 처리는 확인 문구 입력·`admin_audit_log` 에 `game_record_void`/`game_record_restore`). 아래 "메달 현황" 은 `game_top_players(20)` — 명예의 전당과 같은 숫자. "회원" 탭의 **순위 제외** 절은 업로드 차단과 같은 모양(이름 검색 → 제외/해제, 제외 계정 목록, `admin_set_ranking_excluded`, 기록 `ranking_exclude`/`ranking_include`). 두 기능 모두 행을 지우지 않는다 — 삭제 UI 는 만들지 않았다(13절의 정신: 되돌릴 수 있는 조치만).
- **픽셀 판 (2026.9.20, `supabase_pixel_game.sql`)** — "작품 처리" 탭의 "픽셀 판" 절이 판 없는 작품 수·판 있는 작품 수를 보여주고, "판 생성"(없는 것만)·"전체 다시 생성"(확인 뒤, 모두의 진행 초기화)이 이 브라우저에서 썸네일을 읽어 `set_pixel_board` 로 저장한다. 옵트아웃 작품과 판이 될 수 없는 그림(흰 스캔·단색)은 건너뛰고 건너뛴 수를 토스트로 알린다. 옵션은 "사이트 옵션 → 숫자 색칠 게임" 카드(16절). `js/pixel-board.js` 를 admin.html 이 읽는다.
...
- 현재 옵션: `gameMedalMinPlayers`(메달 최소 기록자 수 1~10, 기본 3 — `game_medal_min_players()` 가 서버에서 읽어 모든 순위 RPC 와 `finish_game` 알림에 적용)·`gameRecentRunsEnabled`(최근 게임 목록, 기본 켜짐 — `game_recent_runs` 안에서도 검사)·`gameRecentRunsCount`(처음 표시 개수 5~50, 기본 10) — `supabase_game_medal_rules.sql` (2026.9.20).
- 현재 옵션: `pixelEasyGrid`(24)·`pixelEasyColors`(10)·`pixelNormalGrid`(40)·`pixelNormalColors`(13)·`pixelHardGrid`(64)·`pixelHardColors`(16) — 색칠 난이도 3단계의 긴 변 칸 수(12~96)와 색 수(4~32). 예전 `pixelGridSize`·`pixelPaletteSize` 를 대체한다(2026.9.20). 바꾼 값은 새로 만드는 판부터 적용되고 기존 판은 관리자 "픽셀 판 → 전체 다시 생성" 으로 바꾼다.
- 현재 옵션: `starsEnabled`(별자리 사용, 기본 켜짐 — 꺼도 이미 받은 별은 남는다. 별은 DB 트리거가 기록하므로 이 옵션은 화면만 가린다) — `supabase_stars.sql` (2026.9.21). 함께 있던 `starsShowWhale` 은 고래를 없애면서 뺐고(2026.9.24), `whaleStarsPerPlayer` 는 그 전에 없앴다(2026.9.22).
- 현재 옵션: `pixelDragWrongMode`(쓸기가 아직 안 칠한 다른 번호 칸의 가운데를 지날 때: `stop` 그 쓸기를 끝냄(기본) / `mark` 탭으로 틀렸을 때와 같은 표시·경고음을 남기고 계속 / `pass` 조용히 지나감 = 예전 동작) — 선택형 옵션이라 관리자 화면에서 `<select class="admin-select" data-setting="키">` 로 묶는다(2026.9.21 부터 `admin.js` 가 select 를 지원: 저장값이 `<option>` 값 중 하나가 아니면 기본값).
- 현재 옵션: `pixelSoundVolume`(색칠 효과음 음량 %, 20~300, 기본 100) — 칸 칠하기·색 완성·작품 완성·틀린 번호 소리의 최대 음량에 곱한다(`beep`, coloring.js). **디지털 신호의 천장은 1.0 이라 그 위로는 "모양" 으로만 커진다** (2026.9.21, 두 번의 "더 크게" 요청): ⓐ 음색을 삼각파에서 기음+배음 5개(`brightTone` — 휴대폰 스피커와 귀가 민감한 1.4~4kHz 에 에너지; 홀수 배음은 sine·짝수는 cosine 으로 두어 같은 최대 진폭에서 RMS 0.67, 삼각파 0.58)로, ⓑ 칸 소리를 0.15초로 늘리고 기본 최대 음량을 0.65~0.7 로, ⓒ 마스터 단을 **소프트 클리퍼**(`audioTarget` — 0.7 까지는 직선, 그 위는 tanh 로 1.0 에 수렴; 빠른 쓸기에서 겹친 소리도 찢어지지 않는다)로 두어 150% 이상에서는 신호가 천장을 밀며 촘촘해져 더 크게 들린다(조금씩 거칠어짐). 처음 넣었던 `DynamicsCompressor`(threshold −8dB)는 모든 소리를 같은 크기로 눌러 슬라이더 윗부분이 거의 효과가 없었다 — 되돌리지 말 것. 칸 소리는 35ms 에 한 번으로 제한한다.
- 현재 옵션: `pixelDragDefault`(색칠 입력 기본값 — 켜짐: 고른 번호의 칸을 누른 채 쓸면 이어서 칠하고 두 손가락·다른 칸에서 시작한 드래그는 이동, 꺼짐: 탭으로만 칠하고 한 손가락으로 이동; 플레이어가 ✋ 버튼으로 바꾼 값은 `localStorage` 의 `panMode` 가 우선) (2026.9.20).
- 현재 옵션: `pixelGameEnabled`(숫자 색칠 게임 사용, 기본 켜짐 — **`save_pixel_progress` 안에서도 검사**)·`pixelAnonymousPlay`(비로그인 플레이, 기본 켜짐)·`pixelNotifyArtist`(완성 시 작가 알림, 기본 켜짐 — RPC 가 읽음)·`pixelShowCompletions`(목록에 완성 인원, 기본 켜짐)·`pixelGridSize`(긴 변 칸 수 24~96, 기본 48)·`pixelPaletteSize`(색 수 8~32, 기본 16)·`pixelMergeDistance`(합칠 ΔE 0~20, 기본 8)·`pixelMinColors`(최소 색 수 2~16, 기본 6) — `supabase_pixel_game.sql`. 크기·색 옵션은 새로 만드는 판부터 적용되고 기존 판은 관리자 "픽셀 판 → 전체 다시 생성" 으로 바꾼다(그 판의 진행은 초기화). 기본값은 `pixelBoardOptions()`(pixel-board.js)에도 같은 값으로 있어 옵션 행을 못 읽어도 같은 판이 나온다 (2026.9.20).
