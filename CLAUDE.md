...
- **2026.9.20 추가 SQL**: ⑲ `supabase_portfolios.sql`(작품 공개/비공개 + 작가 포트폴리오: `mosaic_submissions.is_public`(기본 true)과 그 컬럼 grant, 조각이 원작의 공개 여부를 물려받는 트리거, 비공개 전환 시 칸을 돌려주고 공개 전환 시 재시도 표시를 지우는 트리거, **`mosaic_submissions` select 정책 교체**(공개 · 본인 · 관리자 · 공개/링크 공개 포트폴리오에 담긴 작품), `pixel_boards` select 정책(작품이 보일 때만), `match_pool_artworks` 재정의(비공개 조각 제외), `mosaic_stats` 뷰(공개 작품 수), `pixel_user_completions`·`game_medal_artworks`·`game_recent_runs` 재정의(비공개 작품 제외); **⑯·⑰·⑱ 뒤에 실행. ⑯·⑰·⑱ 중 하나라도 다시 실행하면 ⑲ 를 뒤이어 다시 실행한다.** **적용 여부 확인 필요 — 2026.9.20 현재 미적용.**) 미적용이면 업로드의 "비공개" 선택이 "아직 준비되지 않았습니다" 로 거부되고(공개로 몰래 올리지 않는다), 라이트박스 편집 창의 비공개 행이 숨겨지며, 나머지 화면은 예전 그대로 동작한다.
