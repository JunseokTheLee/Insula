...
- **별자리 — 게임의 성취를 남기는 곳** (2026.9.21 사용자 지시, `supabase_stars.sql`·`js/constellations.js`·`js/stars.js`·`css/stars.css`·`{en,ko}/stars.html`). 두 게임 중 하나로 작품을 완성하면 **별 하나**가 켜진다(작품×게임 조합당 하나 — 같은 작품을 다시 해도 늘지 않고, 색칠은 더 높은 단계를 깨면 그 별이 커진다). 별 **6개**가 별자리 하나, 별자리 **6개**가 "밤" 하나다. 모양은 `CONSTELLATION_SHAPES` 12개를 순서대로 돌려 쓰고 밤마다 이름·색이 달라, **작품이 22점에서 100점(별 200개·별자리 33개)으로 늘어도 데이터 파일에 모양만 더하면 된다**(SQL 변경 없음 — 2026.9.21 사용자 확인).
  ...
  - **공동 고래**(`WHALE_STARS` 48개, 머리→등→꼬리→배 순서로 켜져 스스로 그려진다): 누군가 별자리를 완성할 때마다 별 하나가 그 사람 이름으로 켜지되 **1인당 최대 `whaleStarsPerPlayer`(기본 3)개**다. 상한이 없으면 작품이 100점일 때 한 사람이 33개를 켜 고래를 혼자 채운다 — "함께 만든다" 가 깨지므로 넣은 값이다(2026.9.21 사용자 결정). 고래는 저장하지 않고 `user_stars` 에서 계산하므로 별과 어긋날 수 없다.
...
- 현재 옵션: `starsEnabled`(별자리 사용, 기본 켜짐 — 꺼도 이미 받은 별은 남는다. 별은 DB 트리거가 기록하므로 이 옵션은 화면만 가린다)·`starsShowWhale`(함께 만드는 고래 표시, 기본 켜짐)·`whaleStarsPerPlayer`(한 사람이 고래에 켤 수 있는 별 1~12, 기본 3 — **`whale_stars()` 가 서버에서 같은 키를 읽는다**) — `supabase_stars.sql` (2026.9.21).
