...
- 기능: 신고 목록(대상 링크로 열어 한 건씩 검토, 상태 변경만), 캠페인 목록·제목/설명 수정·격자 이미지 생성(이미지 없는 옛 캠페인만)·개별 삭제(작품은 풀로 복귀; 크기·이미지 변경은 캠페인 페이지의 reshape), 관리자 목록(지정·해제는 SQL 안내만), DB·Storage 사용량, 사이트 옵션 체크박스(16절). **13절에 따라 작품 삭제 기능은 여기에 넣지 않는다.**
