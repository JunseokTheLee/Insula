...
- **별자리 — 게임의 성취를 남기는 곳** (2026.9.21 사용자 지시, `supabase_stars.sql`·`js/constellations.js`·`js/stars.js`·`css/stars.css`·`{en,ko}/stars.html`). 두 게임 중 하나로 작품을 완성하면 **별 하나**가 켜진다(작품×게임 조합당 하나 — 같은 작품을 다시 해도 늘지 않고, 색칠은 더 높은 단계를 깨면 그 별이 커진다). 별 **6개**가 별자리 하나, 별자리 **6개**가 "밤" 하나다. 모양은 `CONSTELLATION_SHAPES` 12개를 순서대로 돌려 쓰고 밤마다 이름·색이 달라, **작품이 22점에서 100점(별 200개·별자리 33개)으로 늘어도 데이터 파일에 모양만 더하면 된다**(SQL 변경 없음 — 2026.9.21 사용자 확인).
  ...
  - **화면 구성**(2026.9.21 사용자 지시로 한 번 다시 만들었다): ⓐ **진행 중인 별자리를 가운데 크게** —  한 판(960×600 뷰박스, )에 별자리 하나만 그리고, 패널 비율을 CSS 로 바꿔(모니터 16/10, 휴대폰 1/1·5/6) 좁은 화면에서는 빈 옆면을 잘라낸다 — 폭에 맞춰 축소하면 별이 6px 이 돼 아무것도 안 보였다 ⓑ 그 아래 **칩 줄**()에서 별자리를 골라 무대에 올린다 ⓒ **배지 6개는 각각 전용 도안**(, stars.js: 사람 고리·돋보기와 팔레트·봉우리·수평선 위 별 셋·촘촘한 고리·별 무리) ⓓ **고래가 가장 화려하다** — 채워질수록 밝아지는 실루엣, 완성된 윤곽을 따라 흐르는 빛(), 오로라 띠 2개, 올라가는 물방울, 머리가 그려지면 뜨는 눈.
  - 반짝임은 전부 SVG 필터 + CSS 키프레임이다(라이브러리 없음). 별마다 애니메이션 지연을 다르게 줘 한꺼번에 깜빡이지 않게 하고, ** 에서는 전부 멈춘다**( 맨 아래 목록에 새 애니메이션을 반드시 추가한다). 별 위 카드의 위치는 폭 비율이 아니라  으로 잡는다 — slice 라서 축마다 배율이 다르다.
