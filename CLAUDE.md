...
- **캠페인의 기준 이미지 원본(`reference_image_url`)은 화면에 직접 보여주지 않는다** (2026.9.10 확정). 캠페인이 완성될 때까지 숨겨야 하는 그림이므로, 캠페인을 보여주는 곳(홈 히어로·캠페인 목록·프로필의 참여 캠페인)은 모두 `paintProjectPreview()`(project-preview.js: 빈 칸은 회색, 채워진 칸은 초소형 썸네일)로 그린다. 빈 칸 회색은 `openCellGray()`(common.js)가 `previewContrast` 옵션(기본 40)으로 대비를 낮춰 만든다 — 캠페인 격자와 공유 카드도 같은 함수를 쓴다 (2026.9.10). 새로 캠페인을 노출하는 화면을 만들 때도 같은 렌더러를 쓴다.
...
- **Supabase 요금제: 미확인** (2026.9.10 기준). 대시보드 Organization → Billing 에서 확인해 **여기에 요금제와 월 한도(전송량·DB 용량·Storage·요청 수)를 기록한다.** 확인 전까지는 가장 낮은 요금제(Free: 전송량 월 약 5GB) 기준으로 보수적으로 판단한다.
...
- 현재 옵션: `previewContrast` — 빈 칸 회색의 대비(0~100, 기본 40). 홈·캠페인 카드·캠페인 격자·공유 카드가 `openCellGray()`(common.js)로 그릴 때 적용된다. 공유 카드는 만들 때의 값이 구워지므로 바꾼 뒤에는 관리자 캠페인 목록의 "공유 이미지 다시 생성" 으로 다시 만든다 (2026.9.10).
