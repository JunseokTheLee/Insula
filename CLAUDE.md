...
- **별자리 — 게임의 성취를 남기는 곳** (2026.9.21 사용자 지시, `supabase_stars.sql`·`js/constellations.js`·`js/sky-figures.js`·`js/stars.js`·`css/stars.css`·`{en,ko}/stars.html`). 두 게임 중 하나로 작품을 완성하면 **별 하나**가 켜진다(작품×게임 조합당 하나 — 같은 작품을 다시 해도 늘지 않고, 색칠은 더 높은 단계를 깨면 그 별이 커진다). **2026.9.26 부터 별자리는 이름 있는 13개이고 크기가 제각각이다** — 양자리(별 4개)·돌고래·게·염소·봉황·사자·백조·공작·토끼·황소·페가수스·물고기·전갈자리(17개), 모두 121개가 하늘 하나. **작은 것부터 차례로** 채워지고(사용자 결정), 13개를 다 채우면 같은 13개로 두 번째 하늘이 시작된다. 어느 별이 어느 별자리에 드는지는 **별의 순서만으로** 정해져 DB 에 저장하는 것이 없으므로, `CONSTELLATIONS` 의 순서·크기를 바꾸면 모든 사람의 하늘이 다시 그려진다(SQL 변경 없음).
  ...
  - **화면은 파노라마 한 장이다** (2026.9.26 사용자 지시). 밤하늘 사진 `sky/night.webp`(1916×821, 원본 `sky/ConstellationNightSkyBackground.png` 은 커밋하지 않음)을 화면 높이에 맞추고(PC 78vh/78svh, 760px 이하 84svh; 넓은 화면에서는 끌 거리가 남도록 최대 1.10배까지 키워 사진 위쪽을 자른다) **옆으로 끌어 본다**. 마우스는 포인터 이벤트로 끌고(끈 뒤의 클릭은 막는다), 터치는 기본 가로 스크롤이며, **휠은 가로채지 않는다**(위의 휠 회귀 참고). 13개는 사진 좌표에 놓이고(`sky-figures.js` 의 `at`, 먼저 채우는 넷은 스크롤 없이 보이는 왼쪽, 백조·전갈은 실제처럼 은하수 위, y 640 아래 수평선에는 두지 않음), 아래 캡션 카드가 고른 별자리의 이름·문구·별 칸을 보이며 ‹ › 로 넘긴다. 상태는 셋 — 완성(밝게, 선이 별자리 색) · 채우는 중(중간, 다음 칸이 깜빡임) · 아직(옅은 유령). 끌던 위치는 새로고침·뒤로 가기일 때만 되살리고(`sessionStorage` `weavo.stars.view`), 끌기 안내는 한 번만 보인다(`localStorage` `weavo.stars.dragHint`). 13개를 모두 채운 사람에게는 "n번째 하늘" 버튼이 생긴다. **"크게 보기"** 는 같은 파노라마의 감싸개(`#stSkyWrap`)를 `position:fixed` 로 바꿀 뿐 따로 그리지 않는다(Esc 로 닫힘).
  ...
  - **성능은 이 페이지의 기능 요구다** (2026.9.22 "버벅거린다"). ⓐ **SVG 필터를 하나도 쓰지 않는다** — `feGaussianBlur` 는 감싼 요소가 바뀔 때마다 다시 돌았다. 후광은 `radialGradient`, 별자리의 상태는 그림의 opacity 로만 나타낸다 ⓑ 사진은 `<img>` 한 장, 나머지는 사진 픽셀 좌표의 `<svg>` 한 장이라 크기가 바뀌어도 CSS 크기 두 개만 바뀐다 ⓒ 동물은 비트맵 13장이라 반투명 경로 수백 개보다 다시 칠하기가 싸다 ⓓ 숨 쉬는 별은 페이지당 `ANIM_BUDGET`(46)개까지, 채우는 중인 별자리가 먼저 ⓔ 사진 위의 먼 별은 경로 3개(`.st-tw`)가 opacity 만 바꾼다(2026.9.22 의 흐르는 은하수 `st-way`·`st-haze` 는 사진이 은하수를 대신하면서 걷어냈다). 페이지 배경(`st-field-a/b`·`st-field-dim`)은 `background-position` 이 아니라 `transform` 으로 아주 느리게 흐른다. **여기에 필터를 다시 넣지 말 것.**
