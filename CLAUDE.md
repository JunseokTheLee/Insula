...
- 현재 옵션: `pixelSoundVolume`(색칠 효과음 음량 %, 20~200, 기본 100) — 칸 칠하기·색 완성·작품 완성·틀린 번호 소리의 최대 음량에 곱한다. 2026.9.21 에 기본 음량 자체를 약 3배로 올렸고(칸 0.14→0.45 등 — 휴대폰에서 거의 들리지 않았다), 빠른 쓸기에서 소리가 겹쳐 찢어지지 않도록 모든 소리가 `DynamicsCompressor` 하나를 거치며 칸 소리는 35ms 에 한 번으로 제한한다(`beep`, coloring.js).
