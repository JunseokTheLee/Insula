...
- **2026.9.21 추가 SQL**: ㉒ `supabase_stars.sql`(**별자리** — `user_stars` 테이블(플레이어×작품×게임 한 행, **작품에 FK 를 걸지 않고** 제목·작가 이름을 사본으로 보관), `profiles.stars_hidden`, `game_best_records` insert 와 `pixel_progress` insert/update 에 붙는 별 지급 트리거 2개(`award_star`), 기존 완성 기록 백필, 읽기 RPC `user_star_list`·`artwork_star_counts`·`whale_stars`; **⑯·⑰·⑱·⑲·⑳ 뒤에 실행**. **적용 여부 확인 필요 — 2026.9.21 현재 미적용.**) 미적용이면 `/{lang}/stars` 가 안내문만 보이고 결과 화면의 별 표시·프로필 별자리 줄·작품의 "N명의 하늘에서 빛나는 중" 이 조용히 빠진다(게임 자체는 그대로). **작품에 FK 를 안 건 이유**: 두 게임의 완성 기록은 작품·캠페인이 지워지면 cascade 로 함께 사라지는데, 작가가 그림을 내렸다고 남의 밤하늘이 꺼지면 "남는다" 는 이 기능의 전제가 무너진다. 클라이언트에는 insert 권한이 없어 별을 스스로 줄 수 없다.
...
- **별자리 — 게임의 성취를 남기는 곳** (2026.9.21 사용자 지시, `supabase_stars.sql`·`js/constellations.js`·`js/stars.js`·`css/stars.css`·`{en,ko}/stars.html`). 두 게임 중 하나로 작품을 완성하면 **별 하나**가 켜진다(작품×게임 조합당 하나 — 같은 작품을 다시 해도 늘지 않고, 색칠은 더 높은 단계를 깨면 그 별이 커진다). 별 **6개**가 별자리 하나, 별자리 **6개**가 "밤" 하나다. 모양은 `CONSTELLATION_SHAPES` 12개를 순서대로 돌려 쓰고 밤마다 이름·색이 달라, **작품이 22점에서 100점(별 200개·별자리 33개)으로 늘어도 데이터 파일에 모양만 더하면 된다**(SQL 변경 없음 — 2026.9.21 사용자 확인).
  - **6은 SQL 과 데이터 파일 양쪽에 있다** — `whale_stars()` 의 `n % 6` 과 `STAR_CONSTELLATION_SIZE`. 한쪽만 바꾸면 고래가 개인 별자리와 어긋난다.
  - **고래는 한 사람의 하늘이다** (2026.9.22 사용자 결정 — 처음에는 모두가 함께 만드는 공동 고래였다). `WHALE_STARS` 48점은 **그 사람이 얻은 별을 순서대로** 올린 것이다. 별자리가 이야기의 한 장이라면 고래는 그림 전체이고, 48개를 넘기면 다음 고래가 시작된다. 별 목록 하나로 별자리와 고래를 모두 그리므로 **따로 저장하는 것이 없고** `whale_stars()` RPC 와 `whaleStarsPerPlayer` 옵션은 더 이상 쓰지 않는다(함수는 DB 에 남아 있으나 호출하는 코드가 없다). 고래의 별도 누르면 그 작품이 열린다.
  ...
  - 기본 공개이고 `profiles.stars_hidden` 으로 숨긴다(숨겨도 고래의 별은 이름 없이 남는다). 순위 제외 계정은 고래에서 빠진다. **순위·경쟁에는 쓰지 않는다** — 색칠의 정확성을 서버가 검증할 수 없다는 기존 원칙 그대로다.
  ...
  - **화면 구성**(2026.9.21 사용자 지시로 한 번 다시 만들었다): ⓐ **진행 중인 별자리를 가운데 크게** — `.st-stage` 한 판(960×600 뷰박스, `preserveAspectRatio="xMidYMid slice"`)에 별자리 하나만 그리고, 패널 비율을 CSS 로 바꿔(모니터 16/10, 휴대폰 1/1·5/6) 좁은 화면에서는 빈 옆면을 잘라낸다 — 폭에 맞춰 축소하면 별이 6px 이 돼 아무것도 안 보였다 ⓑ 그 아래 **칩 줄**(`.st-strip`)에서 별자리를 골라 무대에 올린다 ⓒ **고래가 가장 화려하다** — 채워질수록 밝아지는 실루엣(`.st-whale-body`), 완성된 윤곽을 따라 흐르는 빛(`.st-flow`), 오로라 띠 2개, 올라가는 물방울, 머리가 그려지면 뜨는 눈. 휴대폰에서는 고래 패널만 가로로 밀어 크기를 지킨다.
  - 반짝임은 전부 SVG 필터 + CSS 키프레임이다(라이브러리 없음). 별마다 애니메이션 지연을 다르게 줘 한꺼번에 깜빡이지 않게 하고, **`prefers-reduced-motion` 에서는 전부 멈춘다** — `css/stars.css` 맨 아래 목록에 새 애니메이션을 반드시 추가한다. 별 위에 뜨는 작품 카드의 위치는 폭 비율이 아니라 `getScreenCTM()` 으로 잡는다(slice 라서 축마다 배율이 다르다).
  - **전체 화면**(`#stFull`): "전체 별자리 크게 보기" 는 모든 별자리를 화면 모양에 맞춘 격자로 한 번에 그리고, 고래의 "크게 보기" 는 고래만 크게 띄운다. 열 때 그리고 닫을 때 비우므로 닫혀 있는 동안 비용이 0이다. 세로 휴대폰에서는 1.9:1 고래가 커질 수 없어 "가로로 돌리세요" 한 줄을 띄운다.
  - **성능은 이 페이지의 기능 요구다** (2026.9.22 "버벅거린다"). 원인 네 가지를 모두 걷어냈다: ⓐ **SVG 필터를 하나도 쓰지 않는다** — `feGaussianBlur` 는 감싼 요소가 바뀔 때마다 다시 도는데 별이 전부 애니메이션이라 한 프레임에 수백 번 돌았다. 후광은 `radialGradient` 로 대신한다(모양은 같고 비용은 없다) ⓑ 배경 별밭은 `<circle>` 190개가 아니라 **`<path>` 몇 개** ⓒ 페이지 배경의 `filter:blur(18px)` 와 `background-position` 애니메이션 제거(화면 전체를 매 프레임 다시 칠했다) ⓓ 반짝이는 별은 페이지당 `ANIM_BUDGET`(46)개까지만. 결과: 노드 약 700→344, 애니메이션 약 300→29, 필터 0. (그 뒤 아래의 은하수를 넣으며 노드 약 360, 애니메이션 약 45가 됐다 — 전부 transform·opacity 라 프레임마다 다시 그리는 것이 없다.) **여기에 필터를 다시 넣지 말 것.**
  - **배경은 흐르고 별자리는 멈춰 있다** (2026.9.22 사용자 지시 "별자리는 그대로 있고, 배경 별들이 밝기가 변하면서 은하수처럼 살짝 흘러가게"). `scatter()`(stars.js)가 그리는 먼 별들은 ⓐ **밴드 5개**로 나뉘어 저마다 다른 주기로 밝아졌다 어두워지고 ⓑ 그 다섯을 담은 `<g class="st-way">` 가 **딱 한 화면 폭만큼** 왼쪽으로 흐른 뒤 처음으로 돌아간다 ⓒ 별을 `x` 와 `x + 폭` 두 곳에 그려 두어 이음매가 없다(바깥쪽은 SVG 뷰포트가 잘라낸다) ⓓ 별이 흘러갈 자리로 옅은 은하수 띠(`.st-haze`, 그라데이션 타원)를 깔았다. 속도는 폭과 무관하게 초당 7유저단위(`WAY_SPEED`)이므로 무대·고래·전체 화면이 같은 속도로 흐른다. **별자리와 고래의 별·선은 이 그룹 밖에 있어 움직이지 않는다.** 페이지 배경도 같은 원리로 두 층이 서로 반대로 아주 느리게 흐르며 번갈아 밝아진다(`st-field-a/b`·`st-field-dim`) — `background-position` 이 아니라 `transform` 이라 화면을 다시 칠하지 않고, 움직이는 층은 `inset:-64px` 로 작게 잡아 GPU 메모리를 아낀다.
  - **전체 화면이 열려 있는 동안 뒤쪽 애니메이션은 멈춘다** (`html.st-full-open`, 2026.9.22). 덮여서 보이지도 않는 무대·고래·페이지 배경이 계속 돌 이유가 없고, 마침 그때 화면에는 가장 무거운 하늘(별자리 전부)이 떠 있다. `#stFull` 안쪽은 id 선택자가 이겨서 그대로 돈다.
...
- 현재 옵션: `starsEnabled`(별자리 사용, 기본 켜짐 — 꺼도 이미 받은 별은 남는다. 별은 DB 트리거가 기록하므로 이 옵션은 화면만 가린다)·`starsShowWhale`(고래 표시, 기본 켜짐) — `supabase_stars.sql` (2026.9.21). `whaleStarsPerPlayer` 는 고래가 개인 것이 되면서 없앴다 (2026.9.22).
